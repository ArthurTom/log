 ...
 * Created by Wanghaiquan on 2017/3/13.
 */
public class Log {
    ...
    static {
        if (LOG2FILE) {
            processName = getCurProcessName(MyApplication.mContext);
            String sdStatus = Environment.getExternalStorageState();
            if (!sdStatus.equals(Environment.MEDIA_MOUNTED)) {
                android.util.Log.e(TAG, "SD card is not avaiable/writeable right now.");
                LOG2FILE = false;
            }
            File path = new File(MyApplication.PATH);
            if (!path.exists()) {
                android.util.Log.i(TAG, "Create log path:" + MyApplication.PATH);
                path.mkdirs();
            }
            String fileName = "zwklog" + Log.formatTime(System.currentTimeMillis(), "yyyyMMdd") + processName + ".txt";
            file = new File(MyApplication.PATH + File.separator + fileName);
            if (!file.exists()) {
     ...
     * @param s            日志标签
     * @param s1           日志内容
     * @param saveToSdcard 是否要保存到sd卡标志
     */
    ...
    public synchronized static void writeFileToSD(String s, String s1) {
        ...
        try {
            if (!file.exists()) {
                File path = new File(MyApplication.PATH);
                if (!path.exists()) {
                    android.util.Log.d(TAG, "Create log path:" +MyApplication.PATH);
                    path.mkdirs();
                }
                file.createNewFile();
            } else { //每天生成一个日志文件
